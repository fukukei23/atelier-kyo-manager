...
実装作業を開始する前に、以下のドキュメントを必ず読むこと。
経営者の判断が既に記載されている内容について再質問しないこと。

### Layer A — セッション開始時に必ず読む

| ファイル | 内容 | 理由 |
|---|---|---|
| `docs/経営者判断.md` | 手数料・関税率・利益閾値・ブランド戦略・BUYMAルール | 全実装の前提条件 |
| `docs/要件定義.md` | 機能一覧・BUYMA規約・制約事項 | 何を実装するかの定義 |

### Layer B — 該当機能の実装時に読む

| ファイル | 読むタイミング |
|---|---|
| `docs/仕様書.md` | 利益計算・18日ルール・パイプライン実装時 |
| `docs/アーキテクチャ.md` | 新モジュール・DB変更・ルート追加時 |
| `docs/scraping-status.md` | スクレイピング・ブランド価格調査時 |
| `docs/ロードマップ.md` | Phase・Sprint計画・優先度判断時 |
| `docs/adr/` | セキュリティ・脅威モデル関連時 |

### 矛盾時の優先順位

1. `docs/経営者判断.md`（経営者の最新判断が最優先）
2. `docs/要件定義.md` / `docs/仕様書.md`
3. `CLAUDE.md`（本ファイル）
