...
- `results/summary.json`: `watched_total`（監視総数）/ `sold_total`（売却数）/
  ...
  `self_check_ok`（下記セルフチェックの直近結果）。

## セルフチェック（自己診断）

why: 2026-07-03の初回実行で、SOLD判定ロジックが**描画待機不足により常にFalseを返す**
不具合があり、6.5時間分のデータがユーザーの目視確認まで気づかれず無駄になった。
同種の回帰を人間の確認任せにしないため、以下を自動で行う:

- **起動時**: 監視対象キーワードで実際に売り切れ済みの商品を複数件探し、
  `check_sold()` がそのうち1件でも正しく検知できるか検証。**失敗したら実行を開始せず即終了**。
- **定期（約3時間毎・12周ごと）**: 実行中も同じ検証を再実行し、サイト側の変更等による
  回帰を検知。失敗したら**それ以上データを溜めずに実行を停止**。
- 判定は複数候補の**いずれか1件が成功すれば合格**（1件だけを見て判定すると、その1件が
  たまたま「取引中」等の紛らわしい状態や単発の描画ブレに当たっただけで誤って
  「壊れている」と判定してしまう過検知を避けるため）。
- 結果は `summary.json` の `self_check_ok` に反映されるので、ログを遡らなくても
  `results/summary.json` を見るだけで**このデータを信用してよいか**が分かる。
