...
def _normalize_proposal(proposal: dict[str, Any], site: str | None = None) -> dict[str, Any]:
    ...
    raise ValueError("normalize_proposal: 'site' を指定するか、proposal を {<SITE_NAME>: {...}} 形式にしてください。")
...
def extract_suggested_selectors(
    evidence: dict[str, Any] | None, target: str = "pdp.pdp_link_selectors", defaults: list[str] | None = None
) -> list[str] | None:
    if not evidence or not isinstance(evidence, dict):
        return defaults
    selectors_root = evidence.get("suggested_selectors")
    if not isinstance(selectors_root, dict):
        return defaults
...
def apply_selector_proposal(
    *, proposal_path: str | Path, overrides_path: str | Path, site: str
) -> tuple[bool, str | None]:
...
def update_site_selectors(
    *, site: str, new_selectors: dict[str, Any], overrides_path: str | Path
) -> tuple[bool, str | None]:
    """
