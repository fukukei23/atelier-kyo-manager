...
def save_last_error(site: str, kind: str, message: str, screenshot: str | None = None, extra: dict | None = None):
...
class SupplierScout:
    def __init__(
        ...
        self.log.info(
            f"SupplierScout ready: headful={self.headful}, slow_mo={self.slow_mo or 'dynamic'}, speed_up={self.speed_up}, sites={[s.name for s in self.sites]}"
        )
    ...
    def _run_pdp_direct_core(self, browser: Browser, site: SiteConfig, pdp_url: str) -> dict | None:
        page = None
        try:
            ...
            if not title or not price:
                raise RuntimeError("could not parse title or price from PDP body")
    ...
    def _run_site_core(self, context: BrowserContext, site: SiteConfig, query: str) -> dict | None:
        ...
        try:
            if site.force_ui_search or not site.search_template:
                ...
                self._accept_popups(page)
                self._human_wait_random(1000, 2500)
            ...
            if not title or not price_raw:
                raise RuntimeError("could not parse title or price from page")
            currency = detect_currency(price_raw or "", site.currency_hint)
            price = to_number(price_raw or "")
            if not price:
                raise RuntimeError("could not convert price text to number")
    ...
    def run_site_with_context(self, context: BrowserContext, site: SiteConfig, query: str) -> dict:
        ...
        for attempt in range(1, self.attempts + 1):
            try:
                ...
                self.log.warning(
                    f"[{site.name}] TIMEOUT on attempt {attempt}: {e.__class__.__name__} (screenshot: {p})"
                )
            except Exception as e:
                p = ss_path(site.name, f"error_attempt_{attempt}")
                save_last_error(site.name, "ERROR", f"Attempt {attempt}: {e}", p)
                self.log.warning(
                    f"[{site.name}] ERROR on attempt {attempt}: {e.__class__.__name__} (screenshot: {p})",
                    exc_info=False,
                )
    ...
    def _search_ui(self, page, site: SiteConfig, query: str):
        ...
        for sel in site.selectors.search_input or []:
            try:
                ...
                self._human_wait_random()
                input_sel_found = True
                break
            except Exception:
                continue
        if not input_sel_found:
    ...
    def run_once(self, product: str, pdp_url: str | None = None) -> dict:
        results: list[dict] = []
        ...
        def best_key(x):
            return (
                x.get("price_converted") if self.fx_to and x.get("price_converted") is not None else float("inf"),
...
def main():
    ...
    p.add_argument("--sites", nargs="+", help="対象サイト（例: FARFETCH SSENSE）")
    ...
    p.add_argument(
        "--fx-auto", dest="fx_auto", action="store_true", default=True, help="為替レートを自動取得 ※既定: ON"
    )
