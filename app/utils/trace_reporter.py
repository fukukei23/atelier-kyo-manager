...
def _find_all(zf: zipfile.ZipFile, endswith: str, prefix: str | None = None) -> list[str]:
...
def _summarize(events: list[dict[str, Any]], wall_time_start_ms: int | None = None) -> dict[str, Any]:
    """
    イベントリストから統計情報を集計する。
    ★ ユーザー提案のコードでループ全体を置換
    """
...
def make_trace_report(trace_zip: str, out_dir: str | None = None, max_images: int = 6) -> tuple[Path, list[Path]]:
    """
    ...
    with zipfile.ZipFile(zp, "r") as zf:
        # ★ メタデータから起点時刻を取得
        wall_time_start_ms: int | None = None
        try:
