...
class SelectorRepairAgent:
    ...
    async def propose_selector_patches(
        ...
        *,
        ...
        previous_successes: Optional[List[Dict[str, Any]]] = None,  # CR-ATELIER-003 Phase D-10.1 Integration
        previous_failures: Optional[List[Dict[str, Any]]] = None,  # CR-ATELIER-003 Phase D-10.1 Integration
    ) -> Dict[str, Any]:
        """
        ...
        try:
            ...
            logger.info(
                f"[SelectorRepair] Generated {len(normalized.get('candidates', []))} selector candidates"
            )
            ...
            logger.error(
                f"[SelectorRepair] Failed to propose selector patches: {e}",
                exc_info=True
            )
            # エラー時は空の結果を返す
            return {
        ...
        *,
        ...
        if site_constraints:
            ...
            if site_constraints.get("url_patterns"):
                ...
                constraints_section += f"- **URL Path**: パスには以下のいずれかを含むこと:\n"
...
"""
    ...
    def _extract_failed_selector_from_error(self, error_message: str) -> Optional[str]:
        """
    ...
    async def propose_fix(
        ...
        *,
        ...
        """
        logger.warning(
            "[SelectorRepair] propose_fix is deprecated, use propose_selector_patches instead"
        )
