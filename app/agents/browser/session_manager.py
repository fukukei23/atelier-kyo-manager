...
# ---------------------------------------------------------------------------
# Extracted components (same file — internal to SessionManager)
# ---------------------------------------------------------------------------


class StealthConfigurator:
    """BrowserContext のステルス設定・initスクリプトを構築する。"""
    ...
    def build_context_options(self) -> dict[str, Any]:
        ...
        if build_stealth_params_from_site_config is not None:
            stealth_params = build_stealth_params_from_site_config(
                ...
                enable_ua_rotation=self._settings.get("enable_ua_rotation", False),
                enable_viewport_rotation=self._settings.get("enable_viewport_rotation", False),
            )
            ...
            headers = (self._settings.get("extra_http_headers") or {}).copy()
            ...
            if self._settings.get("enable_viewport_rotation"):
            ...
            headers = (self._settings.get("extra_http_headers") or {}).copy()
            headers["Accept-Language"] = self._settings.get("accept_language") or "en-GB,en;q=0.8"
            ctx_opts["extra_http_headers"] = headers

            user_agent = self._settings.get("user_agent")
            if self._settings.get("enable_ua_rotation") and USER_AGENT_POOL:
                user_agent = random.choice(USER_AGENT_POOL)
            if user_agent:
                ctx_opts["user_agent"] = user_agent

        if self._settings.get("enable_har"):
            ctx_opts["record_har_path"] = str(self._run_context.get_path("network.har"))
        if self._settings.get("enable_video"):
            videos_dir = self._run_context.get_path("videos")
    ...
    async def apply_stealth(self, context: BrowserContext) -> None:
        """Stealth モジュールがあれば適用、なければ init スクリプトにフォールバック。"""
        if apply_stealth_to_context is not None and build_stealth_params_from_site_config is not None:
            ...
            if stealth_params is not None:
                ...
                self._log.warning("[StealthConfigurator] Failed to build stealth params, using fallback")
                await self.apply_init_scripts(context)
        else:
            await self.apply_init_scripts(context)

    async def apply_init_scripts(self, context: BrowserContext) -> None:
...
class RequestRouter:
    ...
    async def setup(self, context: BrowserContext) -> None:
        ...
        extra = self._settings.get("extra_block_routes") or []
        ...
        async def _locale_rewrite(route: Route) -> None:
            try:
                ...
                if req.resource_type == "document" and "moncler.com" in url:
                    ...
                    if not is_skippable:
                        fixed = _normalize(url)
                        if fixed != url:
                            log.info(f"[RequestRouter] URL normalized: {url} -> {fixed}")
                            await route.continue_(url=fixed)
                            return
                    else:
                        log.debug(f"[RequestRouter] Skipping locale normalization for path: {path_lower}")
                ...
                log.warning(f"[RequestRouter] route handler error: {e}")
        ...
        def _normalize(url: str) -> str:
            if url_normalizer:
                try:
                    return url_normalizer(url)
                except Exception as e:
                    log.debug(f"[RequestRouter] URL normalizer failed: {e}")
            return url
...
class SessionPersistence:
    ...
    def _get_session_file(self) -> Path:
        ds = self._site_config.get("discovery_settings") or {}
        sess_path = ds.get("session_file")
        if sess_path:
            return Path(sess_path)
        safe_site = "".join(c for c in self._site.lower() if c.isalnum() or c in ("_", "-")) or "default"
    ...
    async def apply(self, context: BrowserContext) -> None:
        ...
        try:
            ...
            self._log.warning(f"[SessionPersistence] Failed to read session file {sess_file}: {e}")
            return
        ...
        if cookies:
            try:
                ...
                self._log.info(f"[SessionPersistence] Restored {len(cookies)} cookies from {sess_file}")
            except Exception as e:
                self._log.warning(f"[SessionPersistence] add_cookies failed: {e}")
        ...
        if local_storage:
            try:
                ...
                self._log.info(
                    f"[SessionPersistence] Restored localStorage keys={list(local_storage.keys())[:5]} from {sess_file}"
                )
            except Exception as e:
                self._log.warning(f"[SessionPersistence] localStorage restore failed: {e}")
...
class SessionManager:
    ...
    def __init__(
        ...
        *,
        ...
        self._stealth = StealthConfigurator(site_config, settings, run_context, self.logger)
        self._persistence = SessionPersistence(site, site_config, self.logger)
        self._router = RequestRouter(settings, url_normalizer, self.logger)
    ...
    async def _ensure_open(self) -> Page:
        ...
        for attempt in range(1, max_attempts + 1):
            ...
            try:
                ...
                context_opts = self._stealth.build_context_options()
                context = await browser.new_context(**context_opts)
                if context is None:
                    raise RuntimeError("Failed to create browser context")

                await self._stealth.apply_stealth(context)
        ...
        await self._router.setup(context)

        if apply_stealth_to_context is None:
            await self._stealth.apply_init_scripts(context)
        ...
        try:
            await self._persistence.apply(context)
