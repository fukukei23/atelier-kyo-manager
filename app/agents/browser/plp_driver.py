...
class PlpNavigationResult:
    ...
    navigation_method: str | None = None  # Stage 4: 追加 ("new_tab", "same_tab", "spa")
...
class PlpDriver:
        ...
        try:
            ...
            if tiles_seen == 0:
                ...
                await self._save_materialize_failure_evidence(
                    ", ".join(plp_config.get("product_tiles", []) + plp_config.get("product_link", [])), plp_config
                )
    ...
    def _get_plp_config(self) -> dict[str, Any]:
        ...
        return {
            ...
            "container": (
                plp_cfg.get("container") or pdp_cfg.get("plp_container_selectors") or ["main", "[role='main']"]
            ),
    ...
    def _get_overlay_config(self) -> dict[str, Any]:
        ...
        return {
            "cookie": {
                "selectors": (
                    overlays_cfg.get("cookie_banner", {}).get("selectors") or ui_cfg.get("cookie_accept") or []
                ),
                "wait_after_click_ms": (overlays_cfg.get("cookie_banner", {}).get("wait_after_click_ms") or 500),
            },
            "geo": {
                "selectors": (
                    overlays_cfg.get("geo_popup", {}).get("selectors") or overlays_cfg.get("geo_modal_selectors") or []
                ),
