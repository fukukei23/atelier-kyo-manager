...
class PlpDriver:
    ...
    def _is_trap_page(self, url: str, trap_config: Dict[str, Any]) -> bool:
        ...
        if self._navigation_driver:
            ...
            return self._navigation_driver._is_trap_page(url, trap_config)

        # URL パターン判定
        url_detect = trap_config.get("detect_by_url", {})
        for pattern in url_detect.get("patterns", []):
            if re.search(pattern, url):
                return True
        for exact in url_detect.get("exact_matches", []):
            if url == exact:
                return True
