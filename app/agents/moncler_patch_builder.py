...
def build_moncler_analysis_payload(
    ...
    self_healing_result: Optional[Dict[str, Any]] = None,
    selector_discovery_result: Optional[Dict[str, Any]] = None,
) -> Dict[str, Any]:
    """
...
def build_moncler_patch_candidate(
    ...
    if rejection_stats.get("double_locale_path", 0) > 0 or rejection_stats.get("trap_pattern", 0) > 0:
        ...
        if new_trap_patterns:
            # 既存のパターンと重複を排除
            unique_new_patterns = [
                p for p in new_trap_patterns
                if p not in current_trap_patterns
            ]
    ...
    if root_cause == "locale redirect loop":
        ...
        patch["risk_assessment"]["notes"] = "ロケールリダイレクトループが検出されました。LocaleGuard の動作を確認してください。"
...
async def process_moncler_self_healing_results(
    ...
    self_healing_result: Optional[Dict[str, Any]] = None,
    selector_discovery_result: Optional[Dict[str, Any]] = None,
    current_site_config: Optional[Dict[str, Any]] = None,
    *,
    generate_markdown: bool = False,
) -> Dict[str, Path]:
    """
    ...
    try:
        ...
        if current_site_config is None:
            try:
                ...
                if overrides_path.exists():
                    ...
                    logger.warning(
                        "[PatchBuilder][Moncler] overrides.local.json not found, "
                        "skipping patch candidate generation"
                    )
                    return {}
            except Exception as e:
                logger.warning(
                    f"[PatchBuilder][Moncler] Failed to load overrides.local.json: {e}",
                    exc_info=True
                )
                return {}
        ...
        logger.error(
            f"[PatchBuilder][Moncler] Failed to process self-healing results: {e}",
            exc_info=True
        )
        return {}
