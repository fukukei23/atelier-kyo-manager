...
try:
    ...
    def extract_product_info(html: str, site_config: Dict) -> Dict: return {}
...
class PriceIntelligenceAgent:
    ...
    def _quit_driver(self):
        if self.driver:
            ...
            except Exception as e: logging.error(f"Error quitting driver: {e}")
    ...
    def _perform_buyma_search(self, brand_name: str, site_config: Dict[str, Any]) -> bool:
        try:
            ...
            if not search_input: raise TimeoutException("Could not find any known search input field.")
    ...
    def run(self, brand_name: str, item_limit: int, site_config: Dict[str, Any], browser_name: str = "chrome") -> List[Dict]:
        if not EXTRACTOR_AVAILABLE: return []

        timeout_sec = site_config.get("discovery_settings", {}).get("timeout_sec", 20)

        try:
            self._init_driver(browser_name, timeout_sec)
            if not self._perform_buyma_search(brand_name, site_config): return []
            ...
            for i in range(6):
                ...
                for item in batch:
                    if (url := item.get("url")) and url not in seen:
                        seen.add(url); anchors.append(item); new_items += 1
                if len(anchors) >= item_limit or (new_items == 0 and i > 1): break
            ...
            for card in anchors[:item_limit]:
                ...
                if pdp_data and pdp_data.get("price"):
                    results.append({
                        "name": pdp_data.get("title") or card.get("name"), "buyma_url": card["url"],
                        "buyma_price": pdp_data.get("price"), "buyma_list_price": pdp_data.get("list_price"),
                        "buyma_discount_pct": pdp_data.get("discount_pct"), "buyma_sale_until": pdp_data.get("sale_until")
                    })
                elif card.get("buyma_price_list_view"):
                    results.append({
                        "name": card.get("name"), "buyma_url": card["url"], "buyma_price": card.get("buyma_price_list_view")
                    })
...
if __name__ == "__main__":
    ...
    try:
        ...
        if not buyma_config: raise RuntimeError("BUYMA config not found.")
    except Exception as e:
    ...
    results = agent.run(
        brand_name=args.brand, item_limit=args.items,
        site_config=buyma_config, browser_name=args.browser
    )
