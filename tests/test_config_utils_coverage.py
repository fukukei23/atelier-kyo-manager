...
class TestAppConfig:
    ...
    def test_config_alias(self):
        from app.config.config import Config, AppConfig
...
class TestConstants:
    ...
    def test_max_lengths(self):
        from app.config.constants import MAX_SUBJECT_LENGTH, MAX_MESSAGE_LENGTH
        assert MAX_SUBJECT_LENGTH == 200
        assert MAX_MESSAGE_LENGTH == 1000

    def test_scraper_constants(self):
        from app.config.constants import SCRAPER_CACHE_SIZE, SCRAPER_REQUEST_TIMEOUT, SCRAPER_MAX_RETRIES
        assert SCRAPER_CACHE_SIZE == 100
        assert SCRAPER_REQUEST_TIMEOUT == 10
        assert SCRAPER_MAX_RETRIES == 3

    def test_fx_constants(self):
        from app.config.constants import FX_REQUEST_TIMEOUT, FX_CACHE_TTL_HOURS
        assert FX_REQUEST_TIMEOUT == 10
        assert FX_CACHE_TTL_HOURS == 12

    def test_llm_constants(self):
        from app.config.constants import LLM_RETRY_BACKOFF_BASE, LLM_CACHE_TTL_SECONDS
...
class TestCostTable:
    ...
    def test_all_category_weight_classes_valid(self):
        from app.config.cost_table import CATEGORY_WEIGHT_CLASS, BUYANDSHIP_SHIPPING_RATES
...
class TestPaths:
    ...
    def test_all_dirs_are_under_project_root(self):
        from app.config.paths import (
            PROJECT_ROOT, SCRIPTS_DEV_DIR, DOCS_REPORTS_DIR,
            LOGS_GENERATED_DIR, TMP_GENERATED_DIR, DATA_GENERATED_DIR,
        )
...
class TestValidation:
    ...
    def test_validate_int_invalid(self):
        from app.utils.validation import validate_int, ValidationError
    ...
    def test_validate_float_invalid(self):
        from app.utils.validation import validate_float, ValidationError
