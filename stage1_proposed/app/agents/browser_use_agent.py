...
try:
    ...
    try:
        ...
        from ..extractors.product_info_extractor import extract_title_price
...
PRODUCT_URL_ALLOW_PATTERNS = re.compile(
    r"/(?:(?:[a-z]{2}-[a-z]{2})/)*"
    r"(?:product(?:s)?|p|pp|prod|shop/[^/]+/(?:p|product))/", re.IGNORECASE
)

PRICE_SELECTORS = [
    "meta[property='product:price:amount']", "meta[itemprop='price']", "[itemprop=price]",
    "span:has(meta[itemprop='price'])", "[data-testid*=price]", "[class*=price]",
]
VISIBLE_PRICE_SELECTORS = [s for s in PRICE_SELECTORS if not s.startswith("meta[")]

SIZE_BUTTON_SELECTORS = [
    "button[aria-disabled='false'][data-size]", "button[aria-disabled='false'][data-testid*='size']",
    "button:not([disabled])[data-size]", "li[data-size] button:not([disabled])",
    "button[class*='size']:not([disabled])", "[role='radiogroup'] [role='radio'][aria-checked='false']",
    "[aria-pressed='false'][class*='size']", "[aria-selected='false'][class*='size']",
]
...
def _is_blocked_host(host: str) -> bool:
    ...
    for bad in EXTERNAL_BLOCKLIST_HOSTS:
        bad = bad.lower()
        if host == bad or host.endswith("." + bad): return True
    return False

def looks_like_product_url(url: str) -> bool:
    try:
        u = urlparse(url)
        if u.scheme in {"mailto", "tel", "javascript", "blob", "data"}: return False
        if not u.path or u.path == "/": return False
        host = u.netloc.lower()
        if _is_blocked_host(host): return False
...
class BrowserUseAgent:
        ...
        try:
            ...
            if likely_plp:
                ...
                base_result = await self._run_pdp_flow(
                    page, site, query, settings, run_context
                )
    ...
    async def _click_size_to_reveal_price(self, page: Page, policy: PDPSizeSelectPolicy, settings: Dict[str, Any]) -> bool:
        ...
        for sel in SIZE_BUTTON_SELECTORS:
            try:
                loc = page.locator(sel)
                if await loc.count() > 0: locators.append(loc)
            except Exception: continue
        if not locators: self.logger.debug("No size buttons found to click (Locator)."); return False

        buttons = locators[0]
        try:
            for loc in locators[1:]: buttons = buttons.or_(loc)
        except AttributeError:
            self.logger.warning("Locator.or_() not found. Falling back to selector string union.")
            try: buttons = page.locator(", ".join(SIZE_BUTTON_SELECTORS))
            except Exception as e_fb: self.logger.error(f"Fallback locator strategy failed: {e_fb}"); return False

        try: count = await buttons.count()
        except Exception as e: self.logger.warning(f"Error counting unified size buttons: {e}"); count = 0
        if count == 0: self.logger.debug("No size buttons found to click (count=0 after union/or_)."); return False

        async def btn_label_at(i: int) -> str:
            b = buttons.nth(i)
            try:
                txt = (await b.inner_text() or "").strip()
                data = await b.get_attribute("data-size"); aria = await b.get_attribute("aria-label")
            ...
            for i in range(count):
                try:
                    ...
                    if lab and any(lab == L for L in prefer):
                        self.logger.debug(f"Clicking preferred size by label: {lab}"); await b.click(); clicked = True; break
                except Exception: continue
        ...
        if not clicked and policy.mode in ("first_instock", "by_label"):
            for i in range(count):
                try:
                    b = buttons.nth(i)
                    if not await b.is_visible() or await b.is_disabled() or await b.get_attribute("aria-disabled") == "true": continue
                    txt = (await b.inner_text() or "").lower()
                    if "out of stock" in txt or "在庫なし" in txt: continue
                    self.logger.debug(f"Clicking first available size (label: {await btn_label_at(i)})"); await b.click(); clicked = True; break
                except Exception: continue
        ...
        if clicked:
            try:
                ...
                await page.wait_for_load_state("networkidle", timeout=2000)
            except Exception: self.logger.debug("networkidle timeout, proceeding...")
            try:
            ...
            except Exception: self.logger.debug("Price selector did not become visible after click (timeout).")
            await page.wait_for_timeout(500)
        return clicked
    ...
    async def _extract_price_with_size_option(self, page: Page, settings: Dict[str, Any]) -> Optional[str]:
        ...
        if await self._click_size_to_reveal_price(page, policy, settings):
            price = await self._read_price_or_none(page)
            if price: self.logger.debug("Price found after size selection."); return price
            else: self.logger.debug("Price NOT found even after size selection.")
        return None
    ...
    async def _extract_from_pdp(self, page: Page, url: str, *, settings: Dict[str, Any], close_after: bool = True, timeout_ms_override: Optional[int] = None) -> Optional[Dict[str, Any]]:
        try:
            ...
            # V88.5.3: (BugFix) `url=` キーワード引数を明示的に指定
            if page.url != url: await page.goto(url=url, wait_until="domcontentloaded", timeout=goto_timeout)
            await self._kill_overlays(page)
            await self._click_continue_shopping_if_present(page, self.runtime_kwargs.get("site_config") or {})
            if settings.get("enable_visual_regression_check") and "pdp" in (settings.get("vrt_scope") or ""): await self._perform_vrt(page, "pdp", settings)
            price_text = await self._extract_price_with_size_option(page, settings)
            if price_text:
                data = await extract_title_price(page) or {}; data["price"] = price_text; data["url"] = page.url
                return data
            else: self.logger.warning(f"[PDP] Price not found (even after size attempts) at: {url}"); return None
        except Exception as e: self.logger.error(f"[PDP] Extraction error at {url}: {e}", exc_info=True); return None
        finally:
            if close_after:
                try:
    ...
    async def _extract_from_pdp_list(self, page: Page, context: BrowserContext, site: str, query: str, pdp_links: List[str], site_config: Dict, settings: Dict, run_context: RunContext, *, start_t: float, budget_ms: int) -> DiscoveryResult:
        original_plp_url = page.url
        limited_urls = pdp_links[:MAX_PDP_LINKS_TO_FOLLOW]
        left_ms = self._time_left_ms(start_t, budget_ms)
        if left_ms <= 0: raise ValueError("Timed out before PDP extraction (watchdog).")
        ...
        async def worker(u: str):
            ...
            try:
                ...
                if left_ms_worker <= 2000: raise asyncio.TimeoutError("Not enough time left for PDP worker.")
                worker_timeout = self._slice_timeout_ms(left_ms_worker, cap_ms=default_worker_cap_ms)
                async with sem:
                    if not context: raise ValueError("BrowserContext is not available for PDP worker")
                    p = await context.new_page(); p.set_default_timeout(worker_timeout)
                    ...
                    if item is None and settings.get("pdp_retry_once", True):
                        ...
                        if left_ms_retry <= 2000: self.logger.warning(f"[PDP Worker] Retrying {u} - no time left."); return None
                        self.logger.info(f"[PDP Worker] Retrying once for {u}..."); await asyncio.sleep(1.0)
                        try:
                            retry_timeout = self._slice_timeout_ms(left_ms_retry - 1000, cap_ms=default_worker_cap_ms)
                            # V88.5.3: (BugFix) `url=` キーワード引数を明示的に指定
                            await p.goto(url=u, wait_until="domcontentloaded", timeout=retry_timeout)
                            item = await self._extract_from_pdp(p, u, settings=settings, close_after=False, timeout_ms_override=retry_timeout)
                        except Exception as retry_e: self.logger.warning(f"PDP worker retry failed for {u}: {retry_e}")
                    return item
            except Exception as e: self.logger.warning(f"PDP worker failed for {u}: {e}"); return e
            finally:
        ...
        if not valid_items: raise ValueError("Found PDP links but price extraction failed after size-selection and retry.")
        run_context.save_json("plp_extracted_items.json", {"extracted_data": valid_items})
        return DiscoveryResult(ok=True, site=site, query=query, message=f"PLP extracted {len(valid_items)} items", evidence={"extracted_data": valid_items, "final_url": original_plp_url})
    ...
    async def _run_plp_flow(self, page: Page, context: BrowserContext, site: str, query: str,
        ...
        if pdp_links: return await self._extract_from_pdp_list(page, context, site, query, pdp_links, site_config, settings, run_context, start_t=start_t, budget_ms=budget_ms)
    ...
    async def _run_pdp_flow(self, page: Page, site: str, query: str, settings: Dict, run_context: RunContext) -> DiscoveryResult:
        ...
        item = await self._extract_from_pdp(page, page.url, settings=settings, close_after=False)
        try: await save_dom(run_context, page, "pdp_dom")
        except Exception as e: logger.warning(f"[Hook C1] Failed: {e}")
        if not item: raise ValueError("Price not found on PDP.")
        run_context.save_json("pdp_extracted_data.json", {"extracted_data": item})
        return DiscoveryResult(ok=True, site=site, query=query, message="PDP extracted", evidence={"extracted_data": item, "final_url": item.get("url")})
