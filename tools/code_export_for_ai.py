# =================================================================================
# File:           C:\Users\USER\tools\atelier-kyo-manager\tools\code_export_for_ai.py
# Version:        v13.1 "Phoenix-AKM" (2025-09-10)
#
# Description:    自律型AIプロジェクトエクスポートツール - atelier-kyo-manager 専用版
#                 - NexusCoreのv13.0をベースに、atelier-kyo-managerプロジェクトへ
#                   特化したプロファイルと設定を導入。
#                 - デフォルトプロファイルを `akm-perplexity` に変更し、即時利用可能に。
#
# Date-Time:      実行日時に基づき、出力ファイル名が動的に生成されます。
# Registry:       このスクリプトはWindowsレジストリを一切使用しません。
#
# 操作するソフト:
...
#   (UI上でプロジェクトルートとプロファイル `akm-perplexity` を選択して実行)
#
# 使用方法 (CLI / 最も推奨):
#   cd C:\Users\USER\tools\atelier-kyo-manager
#   # 専用プロファイルがデフォルトのため、ルート指定のみで実行可能
#   python tools\code_export_for_ai.py .
#
#   # 明示的にプロファイルを指定する場合
#   python tools\code_export_for_ai.py . --profile akm-perplexity
# =================================================================================
...
BASE_COMPRESSION_RATIOS = defaultdict(lambda: 0.5, {
    ...
    ".css": 0.25, ".js": 0.28, ".ts": 0.28, ".tsx": 0.28, ".sh": 0.45,
})
...
COMMON_EXCLUDE_DIRS = [
    ...
    "**/typings",
    # atelier-kyo-manager 固有の除外候補 (必要に応じてコメント解除)
    # "**/data", "**/artifacts", "**/cache", "**/coverage", "**/fixtures", "**/.next"
]

PROFILES: Dict[str, Dict[str, Any]] = {
    # --- atelier-kyo-manager 専用プロファイル ---
    "akm-perplexity": {
        "description": "atelier-kyo-manager 専用: Perplexity 手動アップロード用チャンク",
        "target_mb": 8.0,
        "chunk_target_mb": 2.0,
...
# ============================================================================
class LogSink:
    def __init__(self, logs_dir: Path, dry_run: bool, prefix: str = "akm_export"):
...
def score_files(items: List[FileItem], log: LogSink) -> None:
    ...
    # カスタマイズポイント: atelier-kyo-managerの構造に合わせて重みを調整
    path_weights = {"backend": 3, "frontend": 3, "api": 3, "src": 2, "app": 2, "core": 2}
    name_weights = {"orchestrator": 10, "manager": 9, "main": 8, "app": 8, "api": 5, "routes": 5}
    ext_weights = {".py": 3, ".ts": 3, ".tsx": 3, ".toml": 2, ".yaml": 2, ".md": 1, ".txt": 1}
    log.write_header("ファイルスコアリング")
    for item in items:
        score = 0; s = str(item.rel_path).lower().replace('\\', '/')
...
def collect_and_score_files(root: Path, exclude_globs: Dict[str, List[str]], log: LogSink) -> List[FileItem]:
    ...
    normalized_exclude_files = exclude_globs["files"]
    ...
    for dirpath, dirnames, filenames in os.walk(root, topdown=True):
        ...
        for filename in filenames:
            ...
            if any(fnmatch.fnmatch(filename, p) for p in normalized_exclude_files):
                continue
            try:
...
def select_files(items: List[FileItem], profile: Dict[str, Any], compression_ratios: defaultdict) -> Tuple[List[FileItem], List[Tuple[float, Path]]]:
    ...
    sorted_items = priority_items + sorted(remaining_items, key=lambda x: x.score, reverse=True)
...
def create_report_md(items: List[FileItem], profile_name: str, total_predicted_zip_bytes: float, actual_zip_mb: Optional[float]) -> str:
    ...
    report_lines = ["\n## 4. パッケージ品質レポート (自己診断)"]
    good_exts = {".py", ".md", ".txt", ".json", ".toml", ".yaml", ".yml", ".ini", ".cfg", ".sh", ".bat", ".ts", ".tsx", ".js"}
...
**分析ステップ**:
1.  **歴史の理解 (`PROJECT_CHRONICLE.txt`)**: プロジェクトの進化の歴史と主要な開発テーマを把握してください。
2.  **定量的データの確認 (`MANIFEST_REPORT.md` or `PROJECT_INFO.md`)**: プロジェクトの規模（ファイル数、コード行数）と品質（コード純度、情報密度など）を確認してください。
3.  **ソースコードの分析 (`COMBINED_CODE_*.txt` or `*.zip`)**: ソースコード全体をレビューし、主要なエントリーポイント、設計思想、外部依存関係を特定してください。
        ...
        while not self.stop_event.is_set():
            ...
            if current_commit != last_known_commit:
                ...
                if commit_count_since_last_run >= self.commit_trigger:
                    ...
                    try:
                        # ★★★ atelier-kyo-manager 専用プロファイルを自動実行 ★★★
                        for _ in export_main_generator(str(self.root), "akm-perplexity", False, False, False, DEFAULT_EXPORTS_DIR, DEFAULT_LOGS_DIR, self.stop_event):
                            pass
...
def create_gradio_interface(initial_api_token: Optional[str] = None):
    ...
    stop_event = Event()
    with gr.Blocks(title="Atelier Kyo Manager AI Exporter", theme=gr.themes.Soft()) as demo:
        gr.Markdown("# 自律型AIプロジェクトエクスポートツール v13.1 (AKM-Edition)")
        with gr.Row():
            # デフォルト値をatelier-kyo-managerのパスにすることも可能
            root_tb = gr.Textbox(label="📁 プロジェクトルート", value=str(Path.cwd()))
            profile_dd = gr.Dropdown(choices=[(p["description"], p_name) for p_name, p in PROFILES.items()], value="akm-perplexity", label="📜 プロファイル")
        profile_info = gr.Markdown(value=PROFILES["akm-perplexity"]["description"])
        ...
        def on_profile_change(profile_name: str):
            ...
            is_standard_zip = (mode == "standard_zip")
            return {
                profile_info: gr.Markdown(visible=True, value=f"**選択中**: {profile_data.get('description', '')}"),
                accordion: gr.Accordion(visible=is_standard_zip),
                emit_zip_cb: gr.Checkbox(visible=is_standard_zip),
                emit_folder_cb: gr.Checkbox(visible=is_standard_zip),
            }

        def run_handler(root, profile, emit_zip, emit_folder, dry_run=False, progress=gr.Progress(track_tqdm=True)):
            stop_event.clear()
            ...
            final_log = ""
            for log_update in export_main_generator(root, profile, emit_zip, emit_folder, dry_run, DEFAULT_EXPORTS_DIR, DEFAULT_LOGS_DIR, stop_event, None, progress):
        ...
        all_inputs = [root_tb, profile_dd, emit_zip_cb, emit_folder_cb]
        ...
        profile_dd.change(fn=on_profile_change, inputs=[profile_dd], outputs=[profile_info, accordion, emit_zip_cb, emit_folder_cb])
        demo.load(fn=lambda: on_profile_change("akm-perplexity"), outputs=[profile_info, accordion, emit_zip_cb, emit_folder_cb])
...
def main():
    ...
    parser = argparse.ArgumentParser(description="Atelier Kyo Manager AI Exporter v13.1")
    parser.add_argument("root", nargs="?", default=str(Path.cwd()), help="プロジェクトルート")
    # ★★★ atelier-kyo-manager 専用プロファイルをデフォルトに設定 ★★★
    parser.add_argument("--profile", choices=list(PROFILES.keys()), default="akm-perplexity", help="プロファイル")
    ...
    if args.ui:
        ...
        for _ in export_main_generator(args.root, args.profile, args.emit_zip, args.emit_folder, args.dry_run, DEFAULT_EXPORTS_DIR, DEFAULT_LOGS_DIR, Event()):
            pass
