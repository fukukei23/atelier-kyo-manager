...
def main():
    ...
    parser.add_argument(
        "test_path", nargs="?", default=None, help="テストファイルまたはディレクトリ（デフォルト: tests/）"
    )
    parser.add_argument("-v", "--verbose", action="store_true", default=True, help="詳細出力（デフォルト: True）")
    parser.add_argument("-q", "--quiet", action="store_true", help="簡潔出力（-v を無効化）")
    parser.add_argument("--no-verbose", action="store_true", help="-v フラグを付けない")
    parser.add_argument("extra", nargs=argparse.REMAINDER, help="追加の pytest 引数")
