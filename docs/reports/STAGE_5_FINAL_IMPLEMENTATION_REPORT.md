...
Stage 5「PDP Extractor を site_config 駆動に完全移行」の実装が完了し、仕様に完全準拠した状態になりました。
...
- ✅ 在庫チェックテキスト
  - `availability_patterns` を site_config から取得（重複コードを削除）

- ✅ JSON-LD パス
  - `selectors.pdp.json_ld.paths` から取得（重複コードを削除）
...
以下の箇所でデフォルト値の重複を削除しました：

1. **`_click_size_to_reveal_price()`**: `availability_patterns` のデフォルト値を `_get_pdp_config()` から取得する形に統一
2. **`_extract_from_json_ld_or_meta()`**: JSON-LD パスと Meta タグセレクタのデフォルト値を `_get_pdp_config()` から取得する形に統一

これにより、デフォルト値の設定が `_get_pdp_config()` に一元化され、コードの一貫性が向上しました。

## 変更ファイル一覧

### 実装ファイル

1. **app/agents/browser/product_extractor.py** (約865行)
   - 大幅なリファクタリング完了
...
5. **DRY原則**: デフォルト値の設定を `_get_pdp_config()` に一元化
...
1. **タイトル抽出**: `DEFAULT_TITLE_SELECTORS` → `_get_pdp_config()` に移行
2. **価格抽出**: `DEFAULT_PRICE_SELECTORS` → `_get_pdp_config()` に移行
3. **価格正規化**: 正規表現パターン、strip_chars → `price_rules` に移行
4. **画像 URL 正規化**: `_normalize_image_url()` メソッドに切り出し、`image_base_url` 設定に対応
5. **在庫チェックテキスト**: `availability_patterns` → `_get_pdp_config()` に移行（重複削除）
6. **JSON-LD パス**: `offers.price`, `offers.priceCurrency` → `_get_pdp_config()` に移行（重複削除）
7. **Meta タグフォールバック**: セレクタ → `_get_pdp_config()` に移行（重複削除）
8. **HTML 保存パス**: `raw_html_capture.filename` → `_get_pdp_config()` に移行

### 残存するハードコード（意図的なもの）

1. **`DEFAULT_*_SELECTORS` 定数**: `_get_pdp_config()` 内でフォールバックとして使用（最終的なデフォルト値）
2. **`PriceRules` dataclass**: 型定義として残存（実際の使用は `_get_price_rules()` の戻り値）

## 移行できなかった場合の理由

**なし**: すべてのハードコードが site_config に移行されました。
...
- `docs/reports/STAGE_5_ANALYSIS_AND_FIXES.md` - 分析と修正レポート
- `docs/reports/STAGE_5_COMPLETE_IMPLEMENTATION_REPORT.md` - 実装完了レポート（初期版）
