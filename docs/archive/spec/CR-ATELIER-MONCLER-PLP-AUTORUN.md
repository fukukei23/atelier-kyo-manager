...
### Phase 1: 基盤整備（2025-12-03 現在）

| サブフェーズ | 目標 | 状態 | 完了日 |
|------------|------|------|--------|
| **Phase 1-1: Stealth 共通化** | Bot 検知回避の共通モジュール化 | ✅ 完了 | 2025-12-03 |
| **Phase 1-2: 例外分類・Retry 統一** | BrowserUseAgent の例外処理・retry ロジック統一 | ✅ 完了 | 2025-12-03 |
| **Phase 1-3: Navigation (PLP 修復)** | PLP → PDP 抽出ロジックの修復 | 🔄 次フェーズ | - |
| **Phase 1-4: Instance 再構築** | Moncler instance ディレクトリの再構築 | ✅ 完了 | 2025-12-03 |
| **Phase 1-5: Dry-run 検証** | Stealth / 例外処理の動作確認 | ✅ 完了 | 2025-12-03 |

**Phase 1 の現状サマリー:**
- ✅ **Stealth**: SessionManager 経由で Stealth が適用され、Bot 検知回避が正常に動作
- ✅ **例外分類**: Timeout エラーが適切にキャッチされ、分類されている
- ✅ **Retry**: Materialization の retry が動作している
- ✅ **Proxy**: Proxy が正しく設定され、使用されている
- ✅ **403/429**: Bot 検知によるブロックは発生していない
- ⚠️ **PLP 抽出**: Tile counts は検出されているが、PDP リンク抽出に失敗（0 links）

**次のステップ:**
- **Phase 1-3: PLP 抽出ロジックの修復**（優先度: 高）
  - 詳細は `docs/moncler/PLP_EXTRACTION_FIX_TASK_TEMPLATE.md` を参照
  - DOM snapshot の解析 → site_config の修正 → navigation_driver.py の修正
