...
### ISSUE-114: バックアップ先が CWD 依存のハードコード相対パス（✅ 本PRで修正）

- **真因**: `app/utils/overrides_store.py:36`
  ```python
  BACKUP_DIR = Path(".overrides_backups")   # モジュールレベルの相対パス
  ```
  `_backup_overrides_file(path)` は**引数 `path` がどこにあろうと常に CWD 直下**に書く。

- **症状 (a) — 本番のローテーションが機能しない**: `flask run` / CLI をどのディレクトリから
  起動したかでバックアップの保存先が変わる。`_rotate_backups()` も同じ固定パスを見るため、
  別ディレクトリに散ったバックアップは `MAX_BACKUPS = 50` の対象外になり無限に溜まる。
  「破損した overrides.json を隔離して復旧する」という機能の信頼性が起動場所に依存していた。

- **症状 (b) — テストがリポジトリを汚す**: `tests/test_overrides_store.py` は
  「tmp_path で実際のファイル操作を検証（モック不使用）」と明記しており作りは良いが、
  `tmp_path` が効くのは overrides.json 側だけで、バックアップはリポジトリルートに落ちる。
  `pytest tests/` を1回走らせるだけで `.overrides_backups/*.bak` が6件残っていた。

- **症状 (c) — `.gitignore:189` の行が壊れている（実害あり）**:
  ```
  .overrides_backups/# ONNX model files (generated by ai_model_builder.py)
  ```
  パターンと次セクションのコメントが1行に結合している。`.gitignore` は行末コメントを
  解釈しないため `.overrides_backups/` は無視されず、(b) の副産物が実際に混入していた。
  レビュー時点で **`.bak` 18件（2026-06-09〜06-11 生成）がコミット済み**。
  中身は `{invalid json}`（14バイト）、`{}`（2バイト）等、テストが書き込んだ文字列そのもの。

- **本PRでの対応**:
  1. `BACKUP_DIR` を廃止し、`_backup_dir_for(path) = path.resolve().parent / ".overrides_backups"`
     に変更。バックアップは対象ファイルの隣に置かれ、CWD 依存が消える。
     テストの副産物も `tmp_path` 配下に収まりリポジトリを汚さなくなった。
  2. `_rotate_backups()` に対象ディレクトリを引数で渡し、常に同じ場所を見るようにした。
  3. `.gitignore:189` を2行に分割（保険として残す）。
  4. 追跡済みの `.bak` 18件を `git rm` で削除。
