...
### CI緑化: ruff lint違反修正 + フォーマット統一
- CI の Lint & Format job が ruff 違反（136→142 errors）とフォーマット不統一（47ファイル）で赤状態だったのを解消
- `ruff check --fix`（120件）+ `--unsafe-fixes`（5件）で安全な自動修正（未使用import削除・if文結合・`contextlib.suppress`化・`return`直接化等）
- `ruff format` 適用（47ファイル）
- `ruff.toml` に `[lint.per-file-ignores]` 追加: `__init__.py` の F401（public API再エクスポート）・tests 2件の F841（通知/抽出の副作用検証で戻り値未使用・assert強化は別Issue）
- `navigation_driver.py`: `attempted_recover` に `# noqa: F841`（plp_flow.py と対称なrecover追跡変数・参照未実装で保持）
- `brand_price_scraper.py`: B007（未使用ループ変数 `_key`/`_sub_key` 化）
- 結果: `ruff check` All checks passed / `ruff format --check` 288ファイル整形済み
